 ...
 * @param {Object} opts
 * @param {String} opts.user (optional) - User to subscribe (default is generated from wallet information).
...
API.prototype.pushNotificationsSubscribe = function(opts, cb) {
  var url = '/v1/pushnotifications/subscribe/';
...
API.prototype.pushNotificationsUnsubscribe = function(opts, cb) {
  var url = '/v1/pushnotifications/unsubscribe/';
...
/**
 * Returns subscribed user association.
 * @param {String} user - Subscribed user
 * @return {Callback} cb - Return error if user does not exist
 */
API.prototype.pushNotificationsGetUserAssociations = function(opts, cb) {
  opts.user = opts.user || '';
  var url = '/v1/pushnotifications/users/' + opts.user + '/associations';
 ...
 * @param {String} user - Subscribed user
 * @param {String} alert - Message to user
 * @param {String} badge (optional) - App icon badge count
 * @param {String} sound (optional) - Play this sound on alert
 * @return {Callback} cb - Return error if user does not exist
 */
API.prototype.pushNotificationsSend = function(opts, cb) {
  var url = '/v1/pushnotifications/send/';
  this._doPostRequest(url, opts, function(err, response) {
    if (err) return cb(err);
    return cb(null, response);
  });
